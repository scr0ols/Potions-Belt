...
public class PotionsBeltItem extends Item {
        ...
        if (BeltInventory.hasPotion(belt)) {
            ...
            if (!level.isClientSide()) {
                level.playSound(null, player.getX(), player.getY(), player.getZ(),
                        ModSounds.BOTTLE_OPEN.get(), SoundSource.PLAYERS, 1.0F, 1.0F);
            }
    ...
    public static void openMenu(Player player) {
        ...
        level.playSound(null, player.getX(), player.getY(), player.getZ(),
                ModSounds.BELT_OPEN.get(), SoundSource.PLAYERS, 1.0F, 1.0F);
    }
    ...
    private static void announceNoPotions(Player player, Level level) {
        // Player.displayClientMessage(Component, boolean) was removed in 26.2;
        // sendOverlayMessage(Component) is its direct replacement for the
        // action-bar case (Player's own impl is a no-op, ServerPlayer overrides
        // it to actually send the packet - same polymorphic split as before).
        player.sendOverlayMessage(Component.translatable("potionsbelt.belt.no_potions"));
        level.playSound(null, player.getX(), player.getY(), player.getZ(),
