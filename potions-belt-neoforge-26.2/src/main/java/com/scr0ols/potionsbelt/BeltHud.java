 ...
 *
 * Registered via RegisterGuiLayersEvent, ordered above the vanilla HOTBAR
 * layer -- but on NeoForge 26.2 that ordering only controls render sequence,
 * not visibility (verified against the decompiled GuiLayerManager/Hud
 * source: the vanilla HOTBAR layer's own "hudVisible" (F1 hide-GUI) check
 * is baked into its own wrapped render lambda when Hud registers it, not
 * something layers positioned near it inherit). So this replicates the same
 * check itself via the public Hud.isHidden() accessor
 * (Minecraft.gui.hud.isHidden()), instead of assuming the ordering alone
 * hides this layer together with the hotbar.
 */
